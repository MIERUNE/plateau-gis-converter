...
本ツールでは以下の形式に変換できます。形式によって出力されるファイル構成が異なります。また、変換対象の地物型が複数ある場合（建物と道路など）は地物型ごとに自動でファイル名が決定される形式も存在します。

たとえばGeoPackageであれば複数の地物型を単一ファイルで管理できますが、Shapefileなどは元々複数ファイル出力である上、1ファイルでは1つのレイヤーしか管理できないため、複数地物型を出力すると地物型に応じて自動で命名されます。

![alt text](../resources/use_gui_image-9.png)
...
| KML                       | 単一ファイル      | .kml ファイル                     |
| CZML                      | 単一ファイル      | .czml ファイル                    |
| Minecraft                 | フォルダ          | ワールドデータ（savesフォルダ用） |
| glTF                      | ファイル+フォルダ | .glb/.gltf + テクスチャ           |
| Wavefront OBJ             | 複数ファイル      | .obj, .mtl + テクスチャ           |
...
- OBJでは、平面直角座標系を指定することができます。データに応じて適切な座標系を設定してください。

![alt text](../resources/use_gui_image-10.png)

- また、「モデルの重心を原点に設定」オプションを有効化するとモデルが原点（0, 0）へ移動し、無効にすると元の座標を維持したまま変換されます。他のモデルなどと合わせて利用したい場合には「無効」にすると良いです。

![alt text](../resources/use_gui_image-11.png)
...
単一のCityGMLやフォルダ単位ではなく、正規の構造（G空間情報センターやPLATEAU VIEWからダウンロードしたもの）であればZIPファイルから読み取ることもできます。

![alt text](../resources/use_gui_image-12.png)

画面上部のタブで「ZIPファイルから」を選択するとファイル選択画面に移動します。

![alt text](../resources/use_gui_image-13.png)

ZIPファイルは選択することもできますが、ドラッグ&ドロップすることも可能で、読み込ませると以下のような地図画面が表示されます。

![alt text](../resources/use_gui_image-14.png)

メッシュをクリックすると、対象のメッシュに格納されている地物型が自動で抽出されます。

![alt text](../resources/use_gui_image-16.png)

2次メッシュで広く選択しても良いですし、3次メッシュで細かく範囲を指定しても良いですが、2次メッシュの場合は範囲が広いためそのまま変換すると処理時間やファイルサイズが長くなるので注意してください。

![alt text](../resources/use_gui_image-17.png)

対象メッシュ選択後、必要な地物型を選択し「変換」ボタンを押下します。

![alt text](../resources/use_gui_image-18.png)

その後は「ファイル・フォルダから」変換するのと同様のオプションを選択し、CityGMLを変換することができます。

![alt text](../resources/use_gui_image-19.png)
