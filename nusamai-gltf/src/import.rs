...
fn read_bin_file_(filename: String) -> Result<Vec<u8>, GltfImportError> {
...
fn read_bin_file(uri: String, filename: String) -> Result<Vec<u8>, GltfImportError> {
        ...
        Ok(url) => {
            ...
            match url.scheme() {
                ...
                "file" => {
                    ...
                    match read_bin_file_(String::from(url.path())) {
                        Ok(v) => {
                            return Ok(v);
                        }
                        Err(e) => {
    ...
    // 実行ディレクトリ直下に存在？
    if path.is_file() {
        match read_bin_file_(String::from(path.to_str().unwrap())) {
            Ok(v) => {
                return Ok(v);
            }
            Err(e) => {
    ...
    match pathbuf.parent() {
        Some(_parent) => {
            ...
            parent.push(path);
            match read_bin_file_(String::from(parent.to_str().unwrap())) {
                Ok(v) => {
                    return Ok(v);
                }
                Err(e) => {
...
/// gltf ファイルの読込み
fn import_gltf_(filename: String) -> Result<(Gltf, GltfSeqList), GltfImportError> {
...
/// glb ファイルの読込み
fn import_glb_(filename: String) -> Result<(Gltf, GltfSeqList), GltfImportError> {
    let mut seq_list = GltfSeqList::new();
...
pub fn import_gltf(filename: String) -> Result<(Gltf, GltfSeqList), GltfImportError> {
    ...
    match lcext.as_str() {
        "gltf" => {
            return import_gltf_(filename);
        }
        "glb" => {
            return import_glb_(filename);
        }
        _ => {
