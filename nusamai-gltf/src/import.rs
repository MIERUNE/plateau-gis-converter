...
fn read_bin_file(uri: String, filename: String) -> Result<Vec<u8>, GltfImportError>
{
        ...
        Some(pos) => {
            ...
            if pos == 0 {
                ...
                match BASE64_URL_SAFE.decode(&uri[head.len()..]){
                    ...
                    Err(_e) => {
                        return Result::Err(GltfImportError::OtherError("Base64 buffer decode Error.".to_string()));
                    }
                }
            }
            else{
                // ここに来ることはなさそう
                return Result::Err(GltfImportError::OtherError("Illegal base64 buffer.".to_string()));
            }
        },
        None => {
            // Noop
    ...
    match Url::parse(&uri){
        Ok(url) => {
            ...
            match url.scheme(){
                ...
                _ => {
                    return Err(GltfImportError::OtherError(format!("buffer uri scheme {} not supported.", url.scheme())));
                }
            }
        },
        Err(_e) => {
    ...
    return Err(GltfImportError::OtherError("Unexpected error in read_bin_file().".to_string()));
}
