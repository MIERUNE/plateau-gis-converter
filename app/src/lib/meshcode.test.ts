...
describe('meshcode', () => {
	describe('calculateFirstMeshBounds', () => {
		...
		it('should throw error for invalid mesh code length', () => {
			expect(() => calculateFirstMeshBounds('533')).toThrow('第1次地域区画は4桁以上のメッシュコードが必要です');
		});
	...
	describe('calculateMeshBounds', () => {
		...
		describe('error handling', () => {
			it('should throw error for 5-digit mesh code', () => {
				expect(() => calculateMeshBounds('53393')).toThrow('メッシュコードは6桁（第2次）または8桁（第3次）である必要があります');
			});

			it('should throw error for 7-digit mesh code', () => {
				expect(() => calculateMeshBounds('5339359')).toThrow('メッシュコードは6桁（第2次）または8桁（第3次）である必要があります');
			});
